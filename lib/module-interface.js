...
module.exports = function(leveldb) {
	...
	return {
		...
		getModuleCommit: function(moduleCommitObject, cb) {
			if (!moduleCommitObject || !moduleCommitObject.name || !moduleCommitObject.sha) {
				cb({
					status: 400,
				...
				database.get(moduleCommitObject.name + '/' + moduleCommitObject.sha, function(err, commit) {
					if (err) {
						cb({
							status: 404,
							message: 'Error finding commit "' + moduleCommitObject.name + '/' + moduleCommitObject.sha + '"'
						})
					} else {
						cb(false, commit)
					}
		...
		putModuleCommit: function(moduleCommitObject, cb) {
			...
			if (!moduleCommitObject || !moduleCommitObject.name || !moduleCommitObject.sha || !moduleCommitObject.version || !allFilesAreStrings) {
				...
				database.put(moduleCommitObject.name + '/' + moduleCommitObject.sha, moduleCommitObject, function(err) {
					if (err) {
						cb({
							status: 500,
							message: 'Error adding commit "' + moduleCommitObject.name + '/' + moduleCommitObject.sha + '"'
						})
					} else {
		...
		deleteModuleCommit: function(moduleCommitObject, cb) {
			if (!moduleCommitObject || !moduleCommitObject.name || !moduleCommitObject.sha) {
				...
				database.del(moduleCommitObject.name + '/' + moduleCommitObject.sha, function(err) {
					if (err) {
						cb({
							status: 500,
							message: 'Error deleting commit "' + moduleCommitObject.name + '/' + moduleCommitObject.sha + '"'
						})
					} else {
						cb(false)
					}
